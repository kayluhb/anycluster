...
class MapTools():
    ...
    # "World" in this case means mercator values with shifted
    ...
    def point_MercatorToWorld(self, point):
        ...
        #set the origin of the mercator projected COordinate SYstem
        # to bottom left
        ...
        #and then calculating world to pixels at zoom 0 gives googles
        # world coordinates
        #as it is more common, we use bottomleft as cosy origin

        return point

    #meters/pixel depending on zoom level, 2**zoom gives the number of tiles
    # in a row/column pre zoom level:
    ...
    def cellIDToTileBounds(self, cellID, gridSize):
        ...
        left = int(x) * gridSize
         #miny
        bottom = int(y) * gridSize
        #maxx
        right = (int(x)+1) * gridSize
        #maxy
        top = (int(y)+1) * gridSize

        if self.DEBUG:
            print(
                ' for cell %s%s: left: %s, top: %s, right: %s, bottom: %s' % (
    ...
    Given the topright and bottom left Cell IDs, for example A(10,2) and
    B(15,1) calculate all cells spanned by A and B
    '''
